            ...
            try {
                ...
                .then(data => {
                    if(data.status == 1) {
                        ...
                        const newdata = products.filter(category => category.id != id)
                        setProducts(newdata)
                    }
                    else{
                        console.log(data.message);
            ...
            <div className='col-md-9'>
                <div className='card shadow'>
                    <div className='card-body p-4'>
                        ...
                        {
                            loading == false && products.length == 0 && <div className='text-center py-5'>Categories not found</div>
                        }
                        {
                                        ...
                                        products.map((product, index) => {
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td className='text-center'><img src={product.image_url} alt={product.title} width="70"/></td>
                                                    <td className='text-center'>{product.title}</td>
                                                    <td className='text-center'>${product.price}</td>
                                                    <td className='text-center'>{product.qty}</td>
                                                    <td className='text-center'>{product.sku}</td>
                                                    <td className='text-center'>
                                                        {product.status == 0 ? <span className='badge text-bg-danger'>Blocked</span> : <span className='badge text-bg-success'>Active</span>} 
                                                    </td>
                                                    <td className='text-center'>
                                                        ...
                                                        <Link to="#" className='text-danger ms-2' onClick={()=>deleteProduct(product.id)}>
